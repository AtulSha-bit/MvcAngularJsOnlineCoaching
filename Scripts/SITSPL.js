...
app.controller("MyCtrl", function ($scope, $http, Upload, NgTableParams, $sessionStorage, $scope, $filter, $window) {
    ...
    //This methhod is used in Add course structure(Admin)...
    $scope.Batchdata = function () {
        $http.get('/User/BatchData').then(function (response) {
            //$scope.BatchSizeData = response.data;
            $scope.BatchDetails = response.data;
        });
    };

    $scope.BatchGroupdata = function () {
        $http.get('/User/BatchGroupdata').then(function (response) {
            $scope.BatchGroupData = response.data;
        })
    }
    ...
    $scope.getjoin = function () {
        ...
        $scope.validtoyearpart = moment($scope.CourseValidTo).format("YYYY");
        ...
        if ($scope.JoiningDate != undefined && $scope.Month != undefined && $scope.yearpart <= $scope.validtoyearpart &&
    ...
    $scope.calcuatejoining = function () {
        ...
        if ($scope.JoiningDate != undefined) {
            ...
            $scope.datepart = moment().format("DD");
            $scope.yearpart = moment().format("YYYY");
            $scope.validtoyear = moment($scope.CourseValidTo).format("YYYY");
            ...
            if ($scope.JoiningDate != undefined && $scope.Month != undefined && $scope.yearpart <= $scope.validtoyear &&
                $scope.date2 >= $scope.CourseValidFrom._d && $scope.date2 <= $scope.CourseValidTo._d) {
        ...
        $http.post('/User/ConfirmEmailOtp', { OTP: $scope.VerifyOTP }).then(function (response) {
            ...
            if (response.data == true) {
                ...
                if (sessionStorage.TutorId != undefined) {
                    ...
                    $scope.StudentProfileReg.IsDeleted = Boolean(0);
                    $scope.StudentProfileReg.StudentCategoryId = $scope.StudentCategoryId;
                    $scope.StudentProfileReg.StudentSubCategoryId = $scope.StudentSubCategoryId;
                    $scope.StudentProfileReg.StudentType = "CourseStudent";
                    $scope.StudentProfileReg.BatchId = $scope.BatchId;
    ...
    $scope.editCourseDetail = function (CourseId, StructureId) {
        ...
        $http.get(`/Course/EditCourseDetail?CourseId=${$scope.CourseId}&StructureId=${$scope.StructureId}`).then(
            function (response) {
                ...
                location.href = "/Course/CourseDetails";
            })
    }
    ...
    $scope.courseUpdateInit = function () {
        ...
        if (sessionStorage.CoursesData !== undefined) {
            ...
            $scope.Discount = $scope.data2.Discount;
            $scope.strDuration = $scope.data2.DurationId;
            $scope.DurName = $scope.data2.DurationName;
            $scope.mdlTutorId = $scope.data2.TutorId;
            $scope.mdlBatchSizeCourseStr = $scope.data2.BatchId;
            $scope.StudentCategoryId = $scope.data2.StdCatgId;
            $scope.mdlIsPublishedCourseStru = $scope.IsPublished;
            $scope.mdlBatchSizeCourseStr = $scope.data2.BatchId;
            $scope.SubCategoryId = $scope.data2.StudentSubCategoryId;
            $scope.mdlIsPublishedCourseStru = $scope.data2.IsPublished;
    ...
    $scope.updateCourseDetails = function () {
        ...
        $scope.Duration = $scope.strDuration;
        $scope.Month = $scope.Month;
        $scope.Fees = $scope.Fees;
        $scope.DiscountPer = $scope.Discount;
        ...
        if ($scope.mdlIsPublishedCourseStru != "" && $scope.mdlIsPublishedCourseStru !== undefined) {
            $scope.IsPublished = $scope.mdlIsPublishedCourseStru;
        }
        else {
            $scope.mdlIsPublishedCourseStru = false;
        }

        $scope.BatchId = $scope.mdlBatchSizeCourseStr;
        $scope.StudentSubCategoryId = $scope.SubCategoryId;
        ...
        $http.post('/Course/UpdateCourseDetailsMaster', {
            ...
            ValidTo: $scope.ValidTo, StructureId: $scope.StructureId, DurationId: $scope.strDuration, DurationName: $scope.DurName,
            StudentCategoryId: $scope.StdCatgId, IsPublished: $scope.mdlIsPublishedCourseStru, BatchId: $scope.mdlBatchSizeCourseStr,
            StudentSubCategoryId: $scope.SubCategoryId
        }).then(function (response) {
            $scope.data = response.data;
    ...
    // Get Sub Category Based On Category for Course Structure

    $scope.GetSubCategory = function (id) {
        if ($scope.StudentCategoryId == undefined || $scope.StudentCategoryId == null) {
            $scope.StudentCategoryId = 0;
        }
        $http.get(`/Course/GetSubCategory?CategoryId=${$scope.StudentCategoryId}`).then(function (response) {
    ...
    $scope.saveCourseDetails = function () {
        ...
        $scope.CourseDetails.DurationId = $scope.strDuration;
        $scope.CourseDetails.StdCatgId = $scope.StudentCategoryId;
        $scope.CourseDetails.TutorId = $scope.mdlTutorId;
        $scope.CourseDetails.IsPublished = $scope.mdlIsPublishedCourseStru;
        $scope.CourseDetails.BatchId = $scope.mdlBatchSizeCourseStr;
        $scope.CourseDetails.StudentSubCategoryId = $scope.SubCategoryId;
    ...
    //Commented on 19 Dec 2020 By Dilshad A.
    ...
    $scope.StudentEditDataOnInit = function () {
        ...
        if (sessionStorage.StudentEditData != undefined && sessionStorage.StudentEditData != '') {
            ...
            $http.get(`/Admin/GetStudentEditedData?StudentId=${$scope.Id}`).then(function (response) {
                ...
                $scope.BatchId = $scope.data[0].BatchId;

                if ($scope.data[0].BatchGroupId != "null" && $scope.data[0].BatchGroupId != "") {
                    $scope.BatchGroupId = $scope.data[0].BatchGroupId;
                }
    ...
    $scope.updateStudentsDetails = function () {
        ...
        if ($scope.Name != undefined && $scope.Username != undefined && $scope.Phone != undefined && $scope.Email != undefined && $scope.DOB != undefined && $scope.PaidAmount != undefined &&
            ...
            if ($scope.Namepattern.test($scope.Name) && $scope.emailpattern.test($scope.Email) && $scope.mobilepatterns.test($scope.Phone)) {
                ...
                $scope.updatestudents.BatchId = $scope.BatchId;
                $scope.updatestudents.BatchGroupId = $scope.BatchGroupId;
    ...
    $scope.DeleteDuration = function (Id) {
        $http.post('/Admin/DeleteDuration', {
            ...
            $scope.Durationresdata = response.data;

            if (parseInt(response.data) > 0) {
                alert("Data Deleted Successfully");
                window.location.href = "/Admin/AddDuration";
            }
            else if (response.data == "durationdepend") {
                alert("Can not delete duration due to dependency");
                $("#deleteDurations").modal('hide');
            }
            else {
    ...
    $scope.DeleteInternshipActivityType = function (Id) {
        $http.post('/Admin/DeleteInternshipActivityType', {
            ...
            else if (response.data == "strinternactivitytype") {
                alert("Can not delete internship activity type due to dependency");
                $("#deleteInternActivityType").modal('hide');
            }
            else {
        ...
        $http.post("/Admin/AddMessageForCommunicatioin",
            {
                ...
                lstBrandStudent: $scope.arrBrandStudentByAdmins, IsAllBrandStudent: $scope.chkAllBrandStudents, SubCategoryId: $scope.StudentSubCategoryId,
                BatchId:$scope.BatchId,BatchGroupId:$scope.BatchGroupId
    ...
    $scope.ViewStudentCategoryDetails = function () {
        ...
        $scope.Batchdata()
        $scope.BatchGroupdata()
        $scope.hidebatchForAdminComm = true;
    }

    $scope.showbatchforAdminToStudentComm = function () {
    $scope.hidebatchForAdminComm = false;
    }
    ...
    $scope.ViewSectionByCategoryTypeForStudent = function () {
        if ($scope.mdlCategoryTypeForStudent === "Intern") {
            ...
            $scope.shListOfInternStudent = false;
            $scope.shStudentSubClassCategory = true;
        }
    ...
    // Get Course Based On Categories &&  SubCategories For Admin To Student Comm.
    $scope.GetCourseBasedOnCategoryAndSubCategories = function (StudentSubCategoryId) {
        $scope.SubCategoryId = StudentSubCategoryId;
        $scope.shStudentCourseByStudentClassCategory = true;
        $http.post("/Admin/GetAllCourseByStudentSubCategoryId", { CategoryId: $scope.mdlStudentClassCategory, SubCategoryId: $scope.StudentSubCategoryId }).then(function (response) {
            $scope.AllCourseDataBasedOnSubCategory = response.data;
        })
    }


    // Get Course Based On Category & SubCategories For Tutor To Student Comm
    $scope.GetCourseBasedOnCatgSubCatgForTutToStd = function (StudentSubCategoryId) {
        $scope.SubCategoryId = StudentSubCategoryId;
        $scope.shStudentCourseByStudentClassCategory = true;
        $http.post("/Admin/GetAllCourseByStudentSubCategoryIdForTutor", { CategoryId: $scope.StudentCategoryId, SubCategoryId: $scope.StudentSubCategoryId }).then(function (response) {
            $scope.AllCourseDataBasedOnSubCategoryForTutToStd = response.data;
    ...
    // Get Student details based on Course Id and Category Type For Admin To Student Comm.
    $scope.GetStudentDetailsByCourseId = function (id, categorytype, SubCategoryId,Batch,BatchGroup) {

        if ($scope.mdlCategoryTypeForStudent === "Brand") {
            $http.post("/Admin/GetBrandStudentsByCourseId", { intBrandCourseId: id, categoryid: categorytype }).then(function (response) {
            ...
            $http.post("/Admin/GetStudentsByCourseId", {
                intCourseId: id, categoryid: categorytype, subcategoryid: $scope.StudentSubCategoryId,
            BatchId:$scope.BatchId,BatchGroupId:$scope.BatchGroupId}).then(function (response) {
                ...
                if (response.data != "" && response.data != undefined) {
                    ...
                    $scope.disablesendmessage = false;
                }
                else {
                    $scope.shListOfCourseStudent = false;
                    $scope.disablesendmessage = true;
                }
    ...
    $scope.DeleteStudentCategory = function (Id) {
        $http.post('/Admin/DeleteStudentCategory', {
            ...
            else if (response.data == "strdepend") {
                alert("Can not delete student category due to dependency");
                $("#deleteStudentCatg").modal('hide');
            }
            else {
    ...
    // Add Course Content What Will Learn by Dilshad A. on 22 Dec 2020
    $scope.arrCourseContentWhatWillLearn = [];
    $scope.AddMultipleCourseContentWhatWillLearn = function () {
        if ($scope.mdlLearnPoint !== "" && $scope.mdlLearnPoint !== undefined) {
            $scope.arrCourseContentWhatWillLearn.push({ IsPublished: $scope.mdlWWLIsPublished, LearnPoint: $scope.mdlLearnPoint });
        }
        else {
            alert("Point is mandatory.");
        }
        $scope.mdlWWLIsPublished = "";
        $scope.mdlLearnPoint = "";
    }

    // Delete Course Content What Will Learn by Dilshad A. on 22 Dec 2020
    $scope.arrDeleteCourseWhatWillLearn = [];
    $scope.DeleteCourseContentWhatWillLearn = function (index) {
        var data2 = $scope.arrCourseContentWhatWillLearn[index];
        $scope.arrDeleteCourseWhatWillLearn.push({ WhatWillYouLearnId: data2.WhatWillYouLearnId, Description: data2.LearnPoint, IsPublished: data2.IsPublished });
        $scope.arrCourseContentWhatWillLearn.splice(index, 1);
    }

    // Add Course Content Requirement by Dilshad A. on 23 Dec 2020
    $scope.arrCourseContentRequirement = [];
    $scope.AddMultipleCourseContentRequirement = function () {
        if ($scope.mdlRequirementPoint !== "" && $scope.mdlRequirementPoint !== undefined) {
            $scope.arrCourseContentRequirement.push({ IsPublished: $scope.mdlRequirementIsPublished, RequirementPoint: $scope.mdlRequirementPoint });
        }
        else {
            alert("Point is mandatory.");
        }
        $scope.mdlRequirementsPublished = "";
        $scope.mdlRequirementPoint = "";
    }

    // Delete Course Content Requirement by Dilshad A. on 23 Dec 2020
    $scope.arrDeleteCourseContentRequirement = [];
    $scope.DeleteCourseContentRequirement = function (index) {
        var data2 = $scope.arrCourseContentRequirement[index];
        $scope.arrDeleteCourseContentRequirement.push({ ContentRequirementId: data2.ContentRequirementId, RequirementPoint: data2.RequirementPoint, IsPublished: data2.mdlRequirementIsPublished });
        $scope.arrCourseContentRequirement.splice(index, 1);
    }

    // Add Course Content FAQ by Dilshad A. on 23 Dec 2020
    $scope.arrCourseContentFAQ = [];
    $scope.AddMultipleCourseContentFAQ = function () {
        if ($scope.mdlFAQQuestion !== "" && $scope.mdlFAQQuestion !== undefined && $scope.mdlFAQAnswer !== "" && $scope.mdlFAQAnswer !== undefined) {
            $scope.arrCourseContentFAQ.push({ IsPublished: $scope.mdlFAQIsPublished, Answer: $scope.mdlFAQAnswer, Question: $scope.mdlFAQQuestion });
        }
        else {
            alert("Point is mandatory.");
        }
        $scope.mdlFAQIsPublished = "";
        $scope.mdlFAQQuestion = "";
        $scope.mdlFAQAnswer = "";
    }

    // Delete Course Content FAQ by Dilshad A. on 23 Dec 2020
    $scope.arrDeleteCourseContentFAQ = [];
    $scope.DeleteCourseContentFAQ = function (index) {
        var data2 = $scope.arrCourseContentFAQ[index];
        $scope.arrDeleteCourseContentFAQ.push({ ContentFAQId: data2.ContentFAQId, Answer: data2.Answer, Question: data2.Question, IsPublished: data2.mdlFAQIsPublished, });
        $scope.arrCourseContentFAQ.splice(index, 1);
    }

    // Add Course Content Latest Point by Dilshad A. on 23 Dec 2020
    $scope.arrCourseContentLatestPoint = [];
    $scope.AddMultipleCourseContentLatestPoint = function () {
        if ($scope.mdlLatestPoint !== "" && $scope.mdlLatestPoint !== undefined) {
            $scope.arrCourseContentLatestPoint.push({ IsPublished: $scope.mdlLatestPointIsPublished, LatestPoint: $scope.mdlLatestPoint });
        }
        else {
            alert("Point is mandatory.");
        }
        $scope.mdlLatestPointIsPublished = "";
        $scope.mdlLatestPoint = "";
    }

    // Delete Course Content Latest Point by Dilshad A. on 23 Dec 2020
    $scope.arrDeleteCourseContentLatestPoint = [];
    $scope.DeleteCourseContentLatestPoint = function (index) {
        var data2 = $scope.arrCourseContentLatestPoint[index];
        $scope.arrDeleteCourseContentLatestPoint.push({ ContentLatestId: data2.ContentLatestId, LatestPoint: data2.LatestPoint, IsPublished: data2.mdlLatestPointIsPublished });
        $scope.arrCourseContentLatestPoint.splice(index, 1);
    }
    ...
    $scope.AddCourseContent = function () {
        ...
        $scope.CourseContent.About = $scope.AboutCourse;
        ...
        $http.post('/Admin/AddCourseContentDescription', {
            ...
            arrCoursePrerequisities: $scope.arrCoursePrerequisities, lstWhatWillLearn: $scope.arrCourseContentWhatWillLearn,
            lstRequirements: $scope.arrCourseContentRequirement, lstFAQs: $scope.arrCourseContentFAQ,
            lstLatestPoints: $scope.arrCourseContentLatestPoint
        }).then(function (response) {
            $scope.data = response.data;
    ...
    $scope.updatecoursecontentinit = function () {
        ...
        if (sessionStorage.EditCourseData != '' && sessionStorage.EditCourseData != undefined) {
            ...
            if ($scope.data2["coursecontent"][0] != '' && $scope.data2["coursecontent"][0] != undefined) {
                ...
                $scope.hideaboutcourse = false;
                $scope.AboutCourse = $scope.data2["coursecontent"][0].About;
            ...
            $scope.arrCourseContentWhatWillLearn = [];
            $scope.data5 = [];
            $scope.data5 = $scope.data2["whatwilllearn"];

            if ($scope.data2["whatwilllearn"] !== undefined && $scope.data2["whatwilllearn"] != '') {

                for (var i = 0; i < $scope.data5.length; i++) {
                    $scope.arrCourseContentWhatWillLearn.push({
                        WhatWillYouLearnId: $scope.data5[i].WhatWillYouLearnId, LearnPoint: $scope.data5[i].LearnPoint, IsPublished: $scope.data5[i].IsPublished
                    })
                }
            }
            ...
            $scope.arrCourseContentRequirement = [];
            $scope.data6 = [];
            $scope.data6 = $scope.data2["contentrequirement"];

            if ($scope.data2["contentrequirement"] !== undefined && $scope.data2["contentrequirement"] != '') {

                for (var i = 0; i < $scope.data6.length; i++) {
                    $scope.arrCourseContentRequirement.push({
                        ContentRequirementId: $scope.data6[i].ContentRequirementId, RequirementPoint: $scope.data6[i].RequirementPoint, IsPublished: $scope.data6[i].IsPublished
                    })
                }
            }
            ...
            $scope.arrCourseContentFAQ = [];
            $scope.data7 = [];
            $scope.data7 = $scope.data2["contentfaq"];

            if ($scope.data2["contentfaq"] !== undefined && $scope.data2["contentfaq"] != '') {

                for (var i = 0; i < $scope.data7.length; i++) {
                    $scope.arrCourseContentFAQ.push({
                        ContentFAQId: $scope.data7[i].ContentFAQId, Question: $scope.data7[i].Question, Answer: $scope.data7[i].Answer, IsPublished: $scope.data7[i].IsPublished
                    })
                }
            }
            ...
            $scope.arrCourseContentLatestPoint = [];
            $scope.data8 = [];
            $scope.data8 = $scope.data2["latestpoint"];

            if ($scope.data2["latestpoint"] !== undefined && $scope.data2["latestpoint"] != '') {

                for (var i = 0; i < $scope.data8.length; i++) {
                    $scope.arrCourseContentLatestPoint.push({
                        ContentLatestId: $scope.data8[i].ContentLatestId, LatestPoint: $scope.data8[i].LatestPoint, IsPublished: $scope.data8[i].IsPublished
                    })
                }
            }
    ...
    $scope.UpdateCourseContentDescription = function () {
        ...
        $scope.UpdateCourseContents.About = $scope.AboutCourse;
        ...
        $http.post('/Admin/UpdateCourseContentDescription', {
            ...
            lstWhatWillLearn: $scope.arrCourseContentWhatWillLearn,
            lstRequirements: $scope.arrCourseContentRequirement, lstFAQs: $scope.arrCourseContentFAQ,
            lstLatestPoints: $scope.arrCourseContentLatestPoint,
            deleteWhatWillLearnArray: $scope.arrDeleteCourseWhatWillLearn, deleteRequirementArray: $scope.arrDeleteCourseContentRequirement,
            deleteFAQArray: $scope.arrDeleteCourseContentFAQ, deleteLatestPointArray: $scope.arrDeleteCourseContentLatestPoint 
    ...
    $scope.TutorCommToStudentInit = function () {
        ...
        $scope.StudentType = "Course Student";
        $scope.Categorydata()
        $scope.Coursedata()
        $scope.Batchdata()
        $scope.BatchGroupdata()
    ...
    // Tutor To Course Student Communication get course based on category commented
    //$scope.getCourseBasedOnCategory = function (CategoryId) {
    //    $scope.hideCourse = false;
    //    $scope.hidebatch = false;

    //    $http.get(`/Tutor/GetCourseBasedOnCategory?CategoryId=${CategoryId}`).then(function (response) {
    //        $scope.CourseBasedOnCategory = response.data;
    //    });
    //}


    // Tutor To Course Student
    $scope.getCourseStudentsBasedOnBatch = function (BatchGroupId) {

        $http.post('/Tutor/getCourseStudentsBasedOnBatch', {
            CategoryId: $scope.StudentCategoryId, CourseId: $scope.Course, BatchId: $scope.BatchId, BatchGroupId: $scope.BatchGroupId, SubCategoryId: $scope.StudentSubCategoryId
        }).then(function (response) {
            $scope.CourseStudentsBasedOnBatch = response.data;
            $scope.CourseStudentsList = new NgTableParams({}, { dataset: $scope.CourseStudentsBasedOnBatch })
            if (response.data != "" & response.data != undefined) {
                $scope.hideCourseStudent = false;
                $scope.disablesendmsg = false;

            }
            else {
                $scope.hideCourseStudent = true;
                $scope.disablesendmsg = true;
            }
    ...
    $scope.SelectMultipleCourseStudentForTutorStudentCommunication = function () {
        var data = $scope.CourseStudentsBasedOnBatch;
        $scope.arrCourseArrayForTutorStudent = [];
        for (var i = 0; i < $scope.CourseStudentsBasedOnBatch.length; i++) {
            if ($scope.CourseStudentsBasedOnBatch[i].chkCourseStudents == true) {
    ...
    $scope.TutorToCourseStudentIncomingComm = function () {
    ...
    // View Resource For Download Details By Dilshad A. on 04 Dec 2020
    $scope.ViewResourceForDownloadDetails = function () {
        $http.get("/Admin/ViewResourceForDownloadDetails").then(function (res) {
            //$scope.ResourceForDown = res.data;
            $scope.ResourceForDownTable = new NgTableParams({}, { dataset: res.data });
        });
    };

    // Update Public Resource for Download(Active and Dective) Which is added by Brand Tutor By Dilshad A. on 05 Dec 2020
    $scope.UpdateResourseForDownload = function (Id, IsPublished) {
        $http.post("/Admin/UpdateResourseForDownload", { intResourceId: Id, isPublished: IsPublished }).then(function (res) {
            if (res.data == "Updated") {
                alert("Resource has been activated.");
                location.reload();
            }
            else {
                alert("Failed. Somthing went wrong. Please try after some!!!");
            }
        });
    }
    ...
    $scope.deleteActivities = function (Id) {
        $scope.Id = Id;
    }

    // Delete Activity Post 

    $scope.DeleteActivities = function (ActId) {
        $http.post('/Admin/DeleteAllActivities', {
            Id: $scope.Id
        }).then(function (response) {
            $scope.data = response.data;
            ...
            else if (response.data == "stractivitydepend") {
                alert("Can not delete activity due to dependency");
    ...
    $scope.deleteInternStudents = function (Id) {
        $scope.Id = Id;
    }

    // Delete Intern Students 

    $scope.DeleteInternStudentsByAdmin = function (ActId) {
        $http.post('/Admin/DeleteInternStudentsByAdmin', {
            Id: $scope.Id
        }).then(function (response) {
            $scope.InternStudentdata = response.data;

            if (parseInt(response.data) > 0) {
                alert("Data Deleted Successfully");
                window.location.href = "/Admin/InternStudent";
            }
            else if (response.data == "strinternstudentdepend") {
                alert("Can not delete intern student due to dependency");
                $("#deleteInternStudent").modal('hide');
            }
            else {
                alert("Error while deleting intern student");
            }
    ...
    $scope.editBatch = function (Id) {
        $scope.Id = Id;
        sessionStorage.BatchId = JSON.stringify($scope.Id);

        if (sessionStorage.BatchId !== undefined && sessionStorage.BatchId != '') {
            $scope.data2 = JSON.parse(sessionStorage.BatchId);
        }

        if ($scope.data2 !== undefined && $scope.data2 != '') {
            $scope.Description = $scope.data2.Description;
            $scope.BatchSize = $scope.data2.BatchSize;
            $scope.mdlBatchIsPublished = $scope.data2.IsPublished;
            $scope.BatchId = $scope.data2.BatchId;
        }
    }

    // Batch Add
    $scope.saveBatch = function () {
        if (sessionStorage.BatchId !== undefined && sessionStorage.BatchId !== undefined) {
            $scope.data3 = JSON.parse(sessionStorage.BatchId);
            if ($scope.data3 !== undefined && $scope.data3 != '') {
                $scope.BatchUpdate = {};
                $scope.BatchUpdate.BatchSize = $scope.BatchSize;
                $scope.BatchUpdate.BatchType = $scope.Description;
                $scope.BatchUpdate.IsPublished = $scope.mdlBatchIsPublished;
                $scope.BatchUpdate.BatchId = $scope.data3.BatchId;
            }
            $http.post('/Admin/UpdateBatch', { updatebatch: $scope.BatchUpdate }).then(function (response) {
                $scope.data = response.data;
                if ($scope.data == true) {
                    sessionStorage.clear();
                    alert("Batch updated successfully");
                    location.href = "/Admin/AddBatch";
                }
                else if (response.data == false) {
                    sessionStorage.clear();
            ...
            $scope.AddBatch = {};
            $scope.AddBatch.BatchSize = $scope.BatchSize;
            $scope.AddBatch.BatchType = $scope.Description;
            $scope.AddBatch.IsPublished = $scope.mdlBatchIsPublished;

            $http.post('/Admin/SaveBatch', { addBatch: $scope.AddBatch }).then(function (response) {
                $scope.data = response.data;
                ...
                if ($scope.data == true) {
                    ...
                    alert("Batch Size already exists");
                }
                else if (response.data == "empty") {
                    swal({
                        title: 'Error',
                        text: 'Batch Size is mandatory',
                        icon: "warning",
                        dangerMode: "true"
                    });
                }
                    ...
                    alert("Error! Problem while creating Batch Size");
                }
            })
        }
    }

    // Add Batch Init to get data from db
    $scope.addbatchinit = function () {
        $http.get('/Admin/GetBatch').then(function (response) {
            $scope.Batchdata = response.data;
            $scope.ShowBatchdata = new NgTableParams({}, { dataset: $scope.Batchdata });
        })
    }

    // Delete Batch
    $scope.deleteBatchById = function (Id) {
        $scope.Id = Id;
    }
    // Delete Batch 
    $scope.DeleteBatch = function (Id) {
        $http.post('/Admin/DeleteBatch', {
            Id: $scope.Id
        }).then(function (response) {
            $scope.data = response.data;
            ...
            if (parseInt(response.data) > 0) {
                ...
                alert("Can not delete batch due to dependency");
                $("#deleteBatchSize").modal('hide');
            }
            else {
    ...
    // Edit Batch
    $scope.editBatchGroup = function (Id) {
        $scope.Id = Id;
        sessionStorage.BatchGroupId = JSON.stringify($scope.Id);

        if (sessionStorage.BatchGroupId !== undefined && sessionStorage.BatchGroupId != '') {
            $scope.data2 = JSON.parse(sessionStorage.BatchGroupId);
        }

        if ($scope.data2 !== undefined && $scope.data2 != '') {
            $scope.Description = $scope.data2.Description;
            $scope.BatchName = $scope.data2.BatchName;
            $scope.mdlBatchGroupIsPublished = $scope.data2.IsPublished;
            $scope.BatchGroupId = $scope.data2.BatchGroupId;
        }
    }

    // Batch Group Add
    $scope.saveBatchGroup = function () {
        if (sessionStorage.BatchGroupId !== undefined && sessionStorage.BatchGroupId !== undefined) {
            $scope.data3 = JSON.parse(sessionStorage.BatchGroupId);
            if ($scope.data3 !== undefined && $scope.data3 != '') {
                $scope.BatchUpdate = {};
                $scope.BatchUpdate.BatchName = $scope.BatchName;
                $scope.BatchUpdate.BatchDescription = $scope.Description;
                $scope.BatchUpdate.IsPublished = $scope.mdlBatchGroupIsPublished;
                $scope.BatchUpdate.BatchGroupId = $scope.data3.BatchGroupId;
            }
            $http.post('/Admin/UpdateBatchGroup', { updatebatch: $scope.BatchUpdate }).then(function (response) {
                $scope.data = response.data;
                if ($scope.data == true) {
                    sessionStorage.clear();
                    alert("Batch updated successfully");
                    location.href = "/Admin/AddBatchGroup";
                }
                else if (response.data == false) {
                    sessionStorage.clear();
                    alert("Batch already exists");
            ...
            $scope.AddBatch = {};
            $scope.AddBatch.BatchName = $scope.BatchName;
            $scope.AddBatch.BatchDescription = $scope.Description;
            $scope.AddBatch.IsPublished = $scope.mdlBatchGroupIsPublished;

            $http.post('/Admin/SaveBatchGroup', { addBatchGroup: $scope.AddBatch }).then(function (response) {
                $scope.data = response.data;
                ...
                if ($scope.data == true) {
                    ...
                    alert("Error! Problem while creating Batch Name");
                }
            })
        }
    }
    ...
    // Add Batch Init to get data from db
    $scope.addbatchgroupinit = function () {
        $http.get('/Admin/GetBatchGroup').then(function (response) {
            $scope.Batchdata = response.data;
            $scope.ShowBatchdata = new NgTableParams({}, { dataset: $scope.Batchdata });
        })
    }
    ...
    $scope.deleteBatchGroupById = function (Id) {
        $scope.Id = Id;
    }

    // Delete Batch Group
    $scope.DeleteBatchGroup = function (Id) {
        $http.post('/Admin/DeleteBatchGroup', {
            Id: $scope.Id
        }).then(function (response) {
            $scope.data = response.data;
            ...
            if (parseInt(response.data) > 0) {
                ...
                window.location.href = "/Admin/AddBatchGroup";
            }
            else if (response.data == "strdepend") {
                alert("Can not delete batch name due to dependency");
                $("#deleteBatchSize").modal('hide');
            }
            else {
                alert("Error while deleting Batch Group");
            }
        })
    }

    $scope.studentsubcategoryinit = function () {
        $scope.StudentCategory()
    }

    // Student Category Add

    $scope.saveStudentSubCategory = function () {

        if (sessionStorage.SubCategoryDataOnEdit !== undefined && sessionStorage.SubCategoryDataOnEdit !== undefined) {
            $scope.data3 = JSON.parse(sessionStorage.SubCategoryDataOnEdit);

            if ($scope.data3 !== undefined && $scope.data3 != '') {
                $scope.StudentSubCategoryUpdate = {};
                $scope.StudentSubCategoryUpdate.SubCategoryName = $scope.SubCategoryName;
                $scope.StudentSubCategoryUpdate.Description = $scope.Description;
                $scope.StudentSubCategoryUpdate.IsPublished = $scope.mdlStudentSubCategoryIsPublished;
                $scope.StudentSubCategoryUpdate.StudentCategoryId = $scope.StudentCategoryId;
                $scope.StudentSubCategoryUpdate.StudedetSubCategoryId = $scope.data3.StudedetSubCategoryId;

            }

            $http.post('/Admin/UpdateStudentSubCategory', { updatesubcategory: $scope.StudentSubCategoryUpdate }).then(function (response) {
                $scope.data = response.data;

                if ($scope.data == true) {
                    sessionStorage.clear();
                    alert("Student Sub Category updated successfully");
                    location.href = "/Admin/SubCategory";
                }

                else if (response.data == false) {
                    sessionStorage.clear();
                    alert("Student Sub Category already exists");
                }
                else {
                    alert("Error while Updating Student Sub Category");
                }
            ...
            $scope.AddStudentSubCategory = {};
            $scope.AddStudentSubCategory.SubCategoryName = $scope.SubCategoryName;
            $scope.AddStudentSubCategory.StudentCategoryId = $scope.StudentCategoryId;
            $scope.AddStudentSubCategory.IsPublished = $scope.mdlStudentSubCategoryIsPublished;
            $scope.AddStudentSubCategory.Description = $scope.Description;
            //    $scope.AddStudentSubCategory.IsPublished = $scope.mdlStudentCategoryIsPublished;

            $http.post('/Admin/SaveStudentSubCategory', { addsubcategory: $scope.AddStudentSubCategory }).then(function (response) {
                $scope.data = response.data;

                if ($scope.data == true) {
                    alert("Student Sub Category created successfully");
                    location.href = "/Admin/SubCategory";
                }
                else if (response.data == false) {
                    alert("Student Sub Category already exists");
                    ...
                    swal({
                        title: 'Error',
                        text: 'Student Sub Category is mandatory',
                        icon: "warning",
                        dangerMode: "true"
                    });
                    ...
                    alert("Error! Problem while creating Student Sub Category");
                }
            })
        }
    }
    ...
    $scope.studentsubcategoryinit = function () {
        ...
        $http.get('/Admin/GetStudentSubCategories').then(function (response) {
            $scope.SubCategories = response.data;

            $scope.ShowSubCategory = new NgTableParams({}, { dataset: $scope.SubCategories });
        })
    }


    $scope.editStudentSubCategory = function (SubCategoryId) {

        $scope.SubCategoryId = SubCategoryId;

        $http.get(`/Admin/EditStudentSubCategory?SubCategoryId=${$scope.SubCategoryId}`).then(function (response) {

            $scope.StudentSubCategoryEditedData = response.data;
            sessionStorage.SubCategoryDataOnEdit = JSON.stringify($scope.StudentSubCategoryEditedData);

            if (sessionStorage.SubCategoryDataOnEdit != "" && sessionStorage.SubCategoryDataOnEdit != undefined) {
                $scope.SubCategory = JSON.parse(sessionStorage.SubCategoryDataOnEdit);
            }

            if ($scope.SubCategory !== undefined && $scope.SubCategory != '') {
                $scope.SubCategoryName = $scope.SubCategory.SubCategoryName;
                $scope.Description = $scope.SubCategory.Description;
                $scope.StudentCategoryId = $scope.SubCategory.StudentCategoryId;
                $scope.mdlStudentSubCategoryIsPublished = $scope.SubCategory.IsPublished;
                $scope.StudentCategoryId = $scope.SubCategory.StudentCategoryId;
            }
    ...
    $scope.deleteStudentSubCategoryById = function (Id) {
        $scope.Id = Id;
    }
    ...
    $scope.DeleteStudentSubCategory = function (Id) {
        $http.post('/Admin/DeleteStudentSubCategory', {
            Id: $scope.Id
        }).then(function (response) {
            $scope.data = response.data;

            if (parseInt(response.data) > 0) {
                alert("Student Sub Category Deleted Successfully");
                window.location.href = "/Admin/SubCategory";
            }
            else if (response.data == "strsubcatgdepend") {
                alert("Can not delete student sub category due to dependency");
                $("#deleteStudentSubCatg").modal('hide');
            }
            else {
                alert("Error while deleting Student Sub Category");
            }
        })
    }
    ...
    // Send Course Student msg to Admin

    $scope.SendCourseStudentMsgToAdmin = function () {

        $http.post('/User/SendCourseStudentMsgToAdmin', { MessageToAdmin: $scope.MsgToAdmin }).then(function (response) {
            $scope.CourseStudentMsgToAdmin = response.data;

            if (response.data > 0) {
                alert("Message successfully sent.");
                window.location.href = "/User/CourseStudentToAdminOutbox";
            }
            else {
                console.log(response.data);
                alert("Failed. Something went wrong.");
    ...
    // Get Course Student to Admin Message ( Course Student Outgoing Message To Admin )

    $scope.GetCourseStudentToAdminMessageInit = function () {

     //   GetCourseStudentToAdminMessage
        $http.get('/Admin/GetCourseStudentToAdminMessage').then(function (response) {
            $scope.CourseStudentToAdminMessages = response.data;
            $scope.CourseStudentToAdminOutgoinMsg = new NgTableParams({}, { dataset: $scope.CourseStudentToAdminMessages });
        })

    }

    // Get Brand Student to Admin Message ( Brand Student Outgoing Message To Admin )

    $scope.GetBrandStudentToAdminMessageInit = function () {

        //   GetCourseStudentToAdminMessage
        $http.get('/Admin/GetBrandStudentToAdminMessage').then(function (response) {
            $scope.BrandStudentToAdminMessages = response.data;
            $scope.BrandStudentToAdminOutgoinMsg = new NgTableParams({}, { dataset: $scope.BrandStudentToAdminMessages });
        })

    }


    // Send Course Tutor msg to Admin

    $scope.SendCourseTutorMsgToAdmin = function () {

        $http.post('/User/SendCourseTutorMsgToAdmin', { MessageToAdmin: $scope.MsgToAdminByCourseTutor }).then(function (response) {
            $scope.CourseTutorMsgToAdmin = response.data;

            if (response.data > 0) {
                alert("Message successfully sent.");
                window.location.href = "/User/CourseTutorToAdminOutbox";
            }
            else {
                console.log(response.data);
                alert("Failed. Something went wrong.");
    ...
    // Get Course Tutor to Admin Message ( Course Tutor Outgoing Message To Admin )

    $scope.GetCourseTutorToAdminMessageInit = function () {

        //   GetCourseTutorToAdminMessage
        $http.get('/Admin/GetCourseTutorToAdminMessage').then(function (response) {
            $scope.CourseTutorToAdminMessages = response.data;
            $scope.CourseTutorToAdminOutgoinMsg = new NgTableParams({}, { dataset: $scope.CourseTutorToAdminMessages });
        })
    ...
    $scope.GetBrandTutorToAdminMessageInit = function () {

        //   GetCourseTutorToAdminMessage
        $http.get('/Admin/GetBrandTutorToAdminMessage').then(function (response) {
            $scope.BrandTutorToAdminMessages = response.data;
            $scope.BrandTutorToAdminOutgoinMsg = new NgTableParams({}, { dataset: $scope.BrandTutorToAdminMessages });
        })
